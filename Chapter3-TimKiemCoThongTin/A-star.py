...
def AStar(tree, start, end):
    ...
    while len(frontier) > 0:
        ...
        for child in state.neighbors():
            ...
            if child.get_label() not in [node.get_label() for node in frontier + explored]:
...
if __name__ == "__main__":
    ...
    tree.add_nodes([
        ...
        Node("M", 4),
        Node("N", 0),
        Node("O", 4)  # Corrected the missing Node initialization
    ])
    tree.add_edges([
        ("A", "B", 2),
        ("A", "C", 1),
        ...
        ("C", "H", 3),
        ("D", "I", 2),
        ("D", "J", 4),  # Corrected a syntax error, changed "3" to "J"
        ("F", "K", 2),
        ("F", "L", 1),
        ("F", "M", 4),
        ("H", "N", 2),
        ("H", "O", 4),  # Corrected the missing comma and added a missing edge
    ])
    # tree.nodes[0].coste  # Commented out, as it seems to be incomplete or incorrect
    # print(tree.edges)  # Commented out, as it's not necessary here
    result = AStar(tree, tree.nodes[0], tree.nodes[14])
    if result:
        s = 'explored: '
        for i in result:
            s += i.label
